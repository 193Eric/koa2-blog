...
var dbInit = function () {
  sql.connect(function (err) {
    if (err) {
      ...
      sql.query('CREATE TABLE person(user varchar(255),password varchar(255))', function (err) {
        !err ? console.log('person表创建成功') : console.log(err)
      })
      sql.query('CREATE TABLE blog(name varchar(255),type varchar(255),text longtext)', function (err) {
        !err ? console.log('blog表创建成功') : console.log(err)
      })
      sql.query('CREATE TABLE comment(name varchar(255),text varchar(255))', function (err) {
